...
export default defineComponent({
  ...
  setup(props) {
    ...
    const drawCanvas = () => {
      ...
      const size = Math.min(
        (world.value.clientWidth - 4) / props.modelValue.size,
        (world.value.clientHeight - 4) / props.modelValue.size
      );
      ...
      drawImage(
        context,
        karel,
        props.modelValue.robot.x * size,
        props.modelValue.robot.y * size,
        size,
        size,
        toDegrees()
      );
